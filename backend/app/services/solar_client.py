...
async def get_solar_per_kw(
    lat: float, lon: float, year: int = 2022
) -> np.ndarray:
    url = (
        f"https://power.larc.nasa.gov/api/temporal/hourly/point?"
        f"latitude={lat}&longitude={lon}&start={year}&end={year}"
        f"&community=RE&parameters=ALLSKY_SFC_SW_DWN&format=JSON"
        f"&time-standard=UTC"
    )
    ...
    if arr.shape[0] != 8760:
        raise ValueError(
            f"Expected 8760 hourly points from NASA, got {arr.shape[0]}"
        )
    return arr
